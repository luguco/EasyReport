    ...
    public static void CreateTables(){
        if(MySQLConnection.isConnected()){
            ...
            try {
                ...
                try {
                    ps = MySQLConnection.getConnection().prepareStatement("CREATE TABLE IF NOT EXISTS `ReportedPlayer` (`Player` VARCHAR(100),`UUID` VARCHAR (100),`Reason` VARCHAR(100),`Proof` VARCHAR(100),`Reporter` VARCHAR(100),`Date` TIMESTAMP default CURRENT_TIMESTAMP,`Report_Claimed` VARCHAR(100),`Supporter` VARCHAR(100))");
                    ps.executeUpdate();
